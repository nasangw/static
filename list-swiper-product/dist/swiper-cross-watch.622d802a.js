...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"js/swiper-cross-watch.js":[function(require,module,exports) {
'use strict';

var strapCode, strapLabel, bodyCode, bodyLabel, productCode, arraySelectedProductCode, arraySelectedProductLabel, swiperHorizontal, swiperVertical, isSelectBody, totalItemCount, horizontalSpaceBetween, jsonData;
...
arraySelectedProductLabel = []; // set total count for items
...
function removeItemInCart(elem) {
  ...
  if (isBody) {
    ...
    isSelectBody = false;
    hideElem('#btnTargetStrap'); // showElem('#btnTargetBody');

    $('#btnTargetBody').show().focus();
  } else {
    itemIndex = getIndexInArray($(elem).data('code'));
    arraySelectedProductCode.splice(itemIndex, 1);
    arraySelectedProductLabel.splice(itemIndex, 1); // hideElem('#btnTargetBody');
...
function matchItemCode() {
  arraySelectedProductCode.sort(); // arraySelectedProductCode.sort(function(a, b) {
  //     return a - b;
  // });
  ...
  for (var key in jsonData) {
    if (arraySelectedProductCode.toLocaleString() === jsonData[key].toLocaleString()) {
      productCode = key;
      alert('상품코드는 ' + productCode + ' 입니다.');
      console.log('상품코드는 ' + productCode + ' 입니다.'); // goodsAddCart(productCode);
...
horizontalSpaceBetween = document.documentElement.offsetWidth > 500 ? 140 : 50; // set swiper for body of watch
...
swiperVertical = new Swiper('#swiperVertical', {
  ...
  // },
  onInit: function onInit(s) {
    var active = $('#swiperVertical').find('.swiper-slide-active'); // strapCode = parseInt(active.data('product-code').substring(3));
...
},{}]},{},["../../../../usr/local/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","js/swiper-cross-watch.js"], null)
//# sourceMappingURL=/swiper-cross-watch.622d802a.js.map